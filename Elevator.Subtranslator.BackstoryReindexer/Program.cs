...
namespace Elevator.Subtranslator.BackstoryReindexer
{
    class Program
    {
		...
		private static void AddNewStories(XDocument translatedStories, IEnumerable<Story> stories)
		{
			...
			{
				if (existingStories.Contains(story.Name)) continue;

				root.Add(new XText("\t"));
				XElement newElement = new XElement(XName.Get(story.Name, last.Name.NamespaceName));
				newElement.Add(last.Nodes());
				newElement.Element("title").Value = story.Title;
				newElement.Element("titleShort").Value = story.TitleShort;
				newElement.Element("desc").Value = story.Description;
